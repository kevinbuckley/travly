...
struct SharingDiagnosticView: View {
    ...
    private var testButtonsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            DiagButton(
                ...
                presentActivity(items: ["Hello from TravAI — pre-share test!" as NSString])
            }
            ...
            DiagButton(
                ...
                log("  hasExistingShare: \(sharingService.existingShare(for: trip) != nil)")
                presentActivity(items: ["Hello from TravAI — post-share test!" as NSString])
            }
            ...
            // C3. Wrapped travai:// with real URL
            DiagButton(
                number: 5,
                title: "Wrapped travai:// (real URL)",
                subtitle: "travai://share?url=<real_encoded_url> as string",
                color: .orange,
                requiresShare: true,
                shareReady: shareURL != nil
            ) {
                ...
                log("TEST C3: Wrapped travai:// with real URL as NSString")
                log("  payload: \"\(text)\"")
                presentActivity(items: [text as NSString])
            }
    ...
    private func buildWrappedURL(from shareURL: URL) -> String {
        ...
        return "travai://share?url=\(encoded)"
    }
